...
test("FG Variety defaults to Value and SKU Count", async () => {
  ...
  expect(screen.getByLabelText("Lifecycle filter")).toBeInTheDocument();
});
...
test("deep dive pages include action and reason columns", async () => {
  ...
  await userEvent.type(screen.getByLabelText("Product L1"), "黑绷带50px");

  expect(screen.getByRole("button", { name: "Clear Product L1 filter" })).toBeInTheDocument();
  expect(screen.getAllByLabelText("X range").length).toBeGreaterThan(0);
  expect(screen.getAllByLabelText("Y range").length).toBeGreaterThan(0);
  ...
  expect(screen.getByRole("button", { name: "Export Channel coverage data" })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Export Sample type data" })).toBeInTheDocument();
  expect(screen.getAllByText(/TRAVEL RETAIL|天猫|京东/).length).toBeGreaterThan(0);
...
test("deep dive Product L1 filter does not constrain portfolio pages", async () => {
  render(<App />);
  const nav = within(screen.getByRole("navigation", { name: "Dashboard pages" }));
  await userEvent.click(nav.getByRole("button", { name: "FG SKU Deep Dive" }));
  await userEvent.type(screen.getByLabelText("Product L1"), "黑绷带50px");
  await userEvent.click(nav.getByRole("button", { name: "Core vs. Tail" }));

  expect(screen.queryByText("Select one Product L1")).not.toBeInTheDocument();
  expect(screen.getByLabelText("KPI & chart metric")).toBeInTheDocument();
});
